...
add_filter(
	...
	function ( $exporters ) {
		$exporters['decronel-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'decronel-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'decronel-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'decronel-leads',
						'group_label' => __( 'Quote requests', 'decronel-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['decronel-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'decronel-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
