...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'decronel-leads' ),
					'singular_name' => __( 'Lead', 'decronel-leads' ),
					'edit_item'     => __( 'Förfrågan', 'decronel-leads' ),
					'search_items'  => __( 'Search leads', 'decronel-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'decronel-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'decronel-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'decronel-leads' ),
			'npl_phone'    => __( 'Phone', 'decronel-leads' ),
			'npl_service'  => __( 'Needs', 'decronel-leads' ),
			'npl_timeline' => __( 'When', 'decronel-leads' ),
			'npl_zip'      => __( 'ZIP', 'decronel-leads' ),
			'npl_source'   => __( 'Source', 'decronel-leads' ),
			'npl_status'   => __( 'Status', 'decronel-leads' ),
			'date'         => __( 'Received', 'decronel-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'decronel-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'decronel-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'decronel-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'decronel-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'decronel-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'decronel-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'decronel-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'decronel-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'decronel-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'decronel-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'decronel-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'decronel-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
