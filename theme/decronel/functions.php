<?php
/**
 * Decron EL AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Decron EL AB Leads" plugin, so leads survive a theme change.
 *
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'decronel', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'decronel' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'decronel' ) ) );

		register_block_pattern_category( 'decronel-sections', array( 'label' => __( 'Decron EL AB sections', 'decronel' ) ) );
		register_block_pattern_category( 'decronel-pages', array( 'label' => __( 'Decron EL AB page layouts', 'decronel' ) ) );
	}
);
